...
try:
    ...
    try:
        ...
        logger.warning("Не удалось установить русскую локаль. Даты могут отображаться на английском.")
...
def restricted(func):
    ...
    async def wrapped(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        ...
        if allowed_id is None or allowed_id == 0:
...
async def process_text_and_update_sheet(text: str, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    for category in set(categories_found):
        ...
        if spreadsheet.update_cell(category, value_to_set):
